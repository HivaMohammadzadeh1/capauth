...
The agent under test is Claude Code itself. CapAuth runs as an MCP server, and Claude Code is
launched with CapAuth as its only tool source, so every tool call it makes is enforced.
...
front of it needs three things. CapAuth gives the agent only those three, for the length of
...
- 1:36. Press Approve once. The merge runs. "Autonomy where it is safe. A person where it matters.
  A wall where it is dangerous. CapAuth also denied a stray slack.post_message the agent tried to
...
- Switch to the measurement. "We tried 72 times to make Claude Code follow an injection, four
  payload styles across Opus, Sonnet, and Haiku, with CapAuth and without. It never followed one.
  That is good news about the model, and it is exactly why a review still needs CapAuth. A review
  does not sign off on a batting average. It signs off on a guarantee. With CapAuth the out-of-lease
...
- CapAuth runs as an MCP server between Claude Code and the tools, in line with the MCP
...
The policy ceiling is per agent identity, the way you scope a service account today. CapAuth adds the
short-lived, task-derived lease under that ceiling, which is the piece a standing role cannot express.

**What about MCP?**
CapAuth is an MCP server. Claude Code is launched with CapAuth as its only tool source, so every call
...
It did, 72 times. A CISO cannot sign a refusal rate. With CapAuth the out-of-lease action is unreachable
whatever the model does, and the same lease stopped real overreach that was not an attack.

**What did you measure?**
A matrix of agent model by injection variant by CapAuth on or off, 72 runs in total. Every cell: zero
...
that CapAuth makes the out-of-lease action unreachable regardless, and it also caught real overreach
...
signs. Dawn Song said to inspect how an agent got its result, not the score. CapAuth is the mechanism
for all three.

1. (0:00) "An agent today holds everything its user can reach. CapAuth gives it a lease for one task:
...
2. (0:20) Scenario ai-org, live, CapAuth on, press Run task. "This is the NEC picture. An AI manager
   gets a task. Watch the lease: delegate, read one thread, file one issue. It hands a worker agent one
   capability for five minutes. CapAuth refuses anything wider than the manager holds. Two leases, one
...
4. (1:30) Measurement slide. "72 runs, three models, four payload styles. No model took the bait.
   Every legitimate task completed. CapAuth still caught overreach that was not an attack: a stray Slack
