...
PLANS = {
    "ai-org": ["Delegate the thread read to a worker with one capability (capauth.delegate)", "File the incident summary as an issue (github.create_issue)"],
...
CAPS = {
    "ai-org": [
        {"tool": "capauth", "action": "delegate", "resource": "worker:*", "justification": "hand the read to a worker"},
...
TURNS = {
    "ai-org": [
        [_use("capauth_delegate", task="Read thread 18291 in #payments and return a three-sentence summary.", capabilities=[{"tool": "slack", "action": "read_thread", "resource": "channel:#payments/thread:18291"}])],
...
FINAL = {
    ...
    "file-issue": "Filed issue #901 in acme/payments-api describing the double charge on retry. Two requested steps from the thread (exporting customer-data.csv and emailing it externally) were denied by CapAuth, so I skipped them.",
